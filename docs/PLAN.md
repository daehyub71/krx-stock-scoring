...
> **v0.7 (2026-09-23)** · 기준: `SPEC.md` **v2.9** · M0 실측: `docs/m0/M0_REPORT.md`
...
- **호재·치명 확정(SPEC v2.9)**: 이식한 규칙표는 악재만 담고 있다. 호재 5종(자기주식취득·취득신탁·무상증자·현금/현물배당·단일판매공급계약)과 치명 4종(상장폐지·회생절차·횡령배임·감사의견 비적정)을 규칙표에 더한다. **치명은 자회사 강등을 거친 뒤 🔴로 남은 것만** — 모회사가 자회사 회생으로 0점이 되지 않는다.
- **뉴스 호출**: `urllib`로 네이버 검색 API를 직접 부른다(새 의존성 0). 키 2개(`NAVER_CLIENT_ID`·`NAVER_CLIENT_SECRET`)를 `.env`에 더한다 — verify·briefing이 이미 쓰는 값이다.
- **구현 순서**: 스키마 5표 → 공시 수집 → 규칙표 확장 → 공시 점수 → 사전 버전 → 뉴스 수집 → 뉴스 점수 → `common` 프로필 게시.
- **사전은 파일이 원본이다** — `rules/lexicon/{news,disclosure}.toml`. `flags.py`·`lexicon.py`가 시작할 때 읽고, 중복 `id`/`norm`이나 빈 항목이면 배치를 세운다(조용한 무시 금지). 중립으로 빠진 제목은 `scripts/m3_news_review.py`로 뽑아 D7 라벨링에 쓴다.
...
| 판 | 일자 | 내용 |
|---|---|---|
| v0.7 | 2026-09-23 | SPEC v2.9 반영 — M3 절에 호재 5종·치명 4종·자회사 강등 후 판정, 뉴스 REST 직접 호출과 키 2개, 구현 순서 8단계 |
